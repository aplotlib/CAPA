...
class AICAPAHelper:
    ...
    def generate_capa_suggestions(self, issue_summary: str, analysis_results: Dict) -> Dict[str, str]:
        ...
        """
        system_prompt = """
        You are a medical device quality expert helping to complete a CAPA form following ISO 13485 and FDA standards.
        Return ONLY a valid JSON object with keys for: "issue_description", "root_cause", "corrective_action", "preventive_action", and "effectiveness_verification_plan".
        """
...
class AIEmailDrafter:
    ...
    def draft_vendor_email(self, goal: str, analysis_results: Dict, sku: str,
        ...
        if english_ability <= 2: language_instruction = "IMPORTANT: Use very simple words and short sentences."
        else: language_instruction = "Use standard professional business English."
        system_prompt = f"""You are a quality manager writing a collaborative email to {vendor_name}. Tone must be reasonable, not accusatory. Recipient's English Ability: {english_ability}/5. {language_instruction} Draft an email to {contact_name}. Propose 1-2 realistic KPIs and a timeline (e.g., "initial analysis within 15 days"). Return only the email text."""
...
class MedicalDeviceClassifier:
    ...
    def classify_device(self, device_description: str) -> Dict[str, str]:
        ...
        system_prompt = """You are an expert in U.S. FDA medical device classification (21 CFR 862-892). Return a single, valid JSON object with keys: "classification", "rationale" (citing regulation number), "risks", and "regulatory_requirements". Return ONLY the valid JSON object."""
        ...
        try:
            ...
            return json.loads(response.choices[0].message.content)
        except Exception as e: return {"error": f"Failed to classify device: {e}"}
...
class RiskAssessmentGenerator:
    ...
    def generate_assessment(self, product_name: str, sku: str, product_description: str) -> str:
        ...
        system_prompt = "You are a certified risk management expert (ISO 14971). Generate a formal risk assessment as a Markdown table. Columns: `Hazard`, `Foreseeable Sequence of Events`, `Hazardous Situation`, `Potential Harm`, `Severity (S)`, `Probability (P)`, `Risk Level`, and `Proposed Mitigation`. Identify 5-7 risks. Use a 1-5 scale for S and P."
...
class UseRelatedRiskAnalyzer:
    ...
    def generate_urra(self, product_name: str, product_description: str, intended_user: str, use_environment: str) -> str:
        if not self.client: return "Error: AI client not initialized."
        system_prompt = "You are a human factors engineering expert (IEC 62366). Generate a formal Use-Related Risk Analysis (URRA) in a Markdown table. Columns: `Use Task`, `Potential Use Error`, `Foreseeable Consequences`, `Potential Harm`, `Severity (S)`, `Probability (P)`, `Risk Level`, `Proposed Mitigation/Design Recommendation`. Identify 5-7 critical use tasks."
...
class AIHumanFactorsHelper:
    ...
    # FIX: This method was added to the class
    @retry_with_backoff()
