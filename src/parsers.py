...
def _robust_read_csv(file: IO[bytes], **kwargs) -> pd.DataFrame:
    try:
        file.seek(0); return pd.read_csv(file, encoding='utf-8-sig', **kwargs)
    except Exception:
        try: file.seek(0); return pd.read_csv(file, encoding='latin1', engine='python', **kwargs)
...
def parse_file(uploaded_file: IO[bytes], filename: str) -> Optional[pd.DataFrame]:
    ...
    if 'odoo' in filename_lower and 'inventory' in filename_lower: return _parse_odoo_forecast(uploaded_file)
    if 'return' in filename_lower: return _parse_pivot_returns(uploaded_file)
