    ...
    with st.spinner("Processing and analyzing files..."):
        ...
        for file in sales_files:
            ...
            if raw_df is not None:
                # Pass the selected period to the standardization function
        ...
        if not st.session_state.sales_df.empty and not st.session_state.returns_df.empty:
            ...
            st.warning("⚠️ Sales or Returns file could not be processed. Please check the file format. The tool expects an Odoo-style sales forecast and a standard returns report.")
...
def display_capa_form():
    ...
    with st.form("capa_form"):
        ...
        if st.form_submit_button("💾 Save CAPA Data", type="primary"):
            capa_data = {'capa_number': capa_number, 'product': product_name, 'sku': sku, 'prepared_by': prepared_by, 'date': date.strftime('%Y-%m-%d'), 'severity': severity, 'issue_description': issue_description, 'root_cause': root_cause, 'corrective_action': corrective_action, 'preventive_action': preventive_action}
            is_valid, issues = validate_capa_data(capa_data)
            if is_valid:
                st.session_state.capa_data = capa_data
                st.success("✅ CAPA data saved and validated!")
            else:
                for issue in issues: st.error(f"Compliance Issue: {issue}")
...
def main():
    ...
    with st.sidebar:
        ...
        # NEW: Dropdown for selecting the report period
        report_period_options = {
            "Last 30 Days": 30, "Last 60 Days": 60, "Last 90 Days": 90,
            "Last 120 Days": 120, "Last 180 Days": 180, "Last 365 Days": 365
        }
        selected_period_label = st.selectbox(
            "Select Sales Report Period", options=list(report_period_options.keys())
        )
