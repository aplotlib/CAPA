...
def display_header():
    st.markdown('<div class="main-header"><h1>🏥 Medical Device CAPA Tool</h1><p>Quality Management System</p></div>', unsafe_allow_html=True)
...
def process_files(sales_file, returns_file, misc_files, target_sku, report_period_days):
    with st.spinner("Processing files..."):
        ...
        if misc_files:
            ...
            if not st.session_state.misc_df.empty: st.success("✅ Miscellaneous files processed.")
...
def display_capa_form():
    ...
    with st.form("capa_form"):
        # CAPA form fields...
        submitted = st.form_submit_button("Save CAPA Data")
        if submitted:
            # Validation and saving logic...
            st.success("CAPA Data Saved.")
...
def main():
    ...
    with st.sidebar:
        ...
        report_period_days = st.number_input("Report Period (Days)", min_value=1, value=30)
        st.markdown("---")
        ...
        if input_method == 'File Upload':
            ...
            misc_files = st.file_uploader("3. Upload Miscellaneous Data", accept_multiple_files=True)
    ...
    with tab1:
        if st.session_state.analysis_results: display_metrics_dashboard(st.session_state.analysis_results)
        else: st.info("Enter data and click 'Process' to see the analysis.")
        if not st.session_state.misc_df.empty:
            st.markdown("---"); st.markdown("### Miscellaneous Uploaded Data"); st.dataframe(st.session_state.misc_df)
    with tab2: display_capa_form()
    with tab3: display_doc_gen()
