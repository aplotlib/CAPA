...
class AIFileParser:
    ...
    Can handle text, CSV, Excel, and image files (via OCR).
    """
    ...
    def _get_file_preview(self, file: IO[bytes]) -> str:
        """
        Creates a text preview of a file (Excel, CSV, TXT) for the AI to analyze.

        Args:
            file: A file-like object in bytes mode.

        Returns:
            A string containing a preview of the file content.
        """
        file.seek(0)
    ...
    def analyze_file_structure(self, file: IO[bytes], target_sku: str) -> Dict[str, Any]:
        ...
        Args:
            file: The file to analyze (as a byte stream).
            target_sku: The target product SKU to look for in the file.

        Returns:
            A dictionary with the analysis results, including content type and key data.
        """
        if not self.client:
        ...
        system_prompt = """
        You are a data analysis expert for a quality management system. Your task is to analyze
        a file preview to determine its content type and extract key data.
        ...
        Based on the preview, complete the following tasks:
        1. Identify the file's primary purpose. Choose one content type from this exact list:
           ['sales', 'returns', 'inspection', 'voice_of_customer', 'other'].
        2. Extract the most relevant data based on the content type:
           - For 'sales' or 'returns', find the total quantity for the target SKU.
           - For 'inspection' files, summarize the pass/fail results.
           - For 'voice_of_customer' files, extract the NCX rate or key customer complaints.
        3. Provide a brief one-sentence summary of the file's content.
        ...
        try:
            ...
            return {"error": "Failed to parse AI response as JSON.", "filename": file.name}
        except Exception as e:
            return {"error": f"An unexpected AI analysis error occurred: {e}", "filename": file.name}

    def extract_data(self, file: IO[bytes], analysis: Dict[str, Any], target_sku: str) -> Optional[pd.DataFrame]:
        """
        Extracts and standardizes data from a file based on the AI analysis.
        This function prioritizes data found by the AI, then falls back to standard parsing.

        Args:
            file: The file to extract data from (as a byte stream).
            analysis: The result from the `analyze_file_structure` method.
            target_sku: The target SKU for which to extract data.

        Returns:
            A pandas DataFrame with 'sku' and 'quantity' columns, or None if not applicable.
        """
        content_type = analysis.get('content_type')
        if content_type not in ['sales', 'returns']:
            return None

        # First, try to use the quantity directly extracted by the AI
        key_data = analysis.get('key_data', {})
        quantity = key_data.get('total_quantity', key_data.get('quantity'))

        if quantity is not None:
            try:
                # Clean and convert the extracted quantity
                clean_quantity = int(float(str(quantity).replace(',', '')))
                return pd.DataFrame([{'sku': target_sku, 'quantity': clean_quantity}])
            except (ValueError, TypeError):
                pass  # If conversion fails, proceed to fallback parsing

        # Fallback: If AI didn't find a quantity, parse the whole file
        file.seek(0)
        try:
            ...
            # Dynamically find SKU and quantity columns
